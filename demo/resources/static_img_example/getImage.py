...
z_s = np.sqrt((box[2] + (box[2]+box[3])/2) * (box[3] + (box[2]+box[3])/2))
...
x_s = 303 / scale
dx, dy = -50, 20
target_bbox = tuple(map(int, (box[0]-z_s/2, box[1]-z_s/2, box[0]+z_s/2, box[1]+z_s/2)))
search_bbox = tuple(map(int, (box[0]-x_s/2+dx, box[1]-x_s/2+dy, box[0]+x_s/2+dx, box[1]+x_s/2+dy)))

im_z = cv2.resize(im[target_bbox[1]:target_bbox[3], target_bbox[0]:target_bbox[2], :], (z_size, z_size))
im_x = cv2.resize(im[search_bbox[1]:search_bbox[3], search_bbox[0]:search_bbox[2], :], (x_size, x_size))
