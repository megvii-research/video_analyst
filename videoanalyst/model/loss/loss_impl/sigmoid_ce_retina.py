...
class SigmoidCrossEntropyRetina(ModuleBase):
    ...
    def update_params(self,):
        ...
        self.register_buffer("alpha", torch.tensor(float(self._hyper_params["alpha"]), requires_grad=False))
        self.register_buffer("gamma", torch.tensor(float(self._hyper_params["gamma"]), requires_grad=False))
    ...
    def forward(self, pred_data, target_data):
        ...
        loss = loss.sum() / torch.max(positive_mask.sum(), self.t_one)
        extra = dict()
