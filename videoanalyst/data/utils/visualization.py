...
def show_img_FCOS(cfg, training_data, distractor_boxes_recentered=[], dataset='untitled'):
    ...
    gt_datas = [training_data["cls_gt"], 
                training_data["ctr_gt"], 
    ...
    if gt_indexes.any():
        ...
        cv2.rectangle(show_img, (int(gt[0]), int(gt[1])), (int(gt[2]), int(gt[3])), color['pos'])
    ...
    for index in pos_indexes:
        ...
        ctr = ctr_gt[index]
        color_pos = tuple((np.array(color['pos']) + ctr * np.array(color['ctr'])).astype(np.uint8).tolist())
    ...
    for index in neg_indexes:
        ...
        pos = (score_offset + (index % score_size) * total_stride,
               score_offset + (index // score_size) * total_stride)
        ctr = ctr_gt[index]
        color_neg = tuple((np.array(color['neg']) + ctr * np.array(color['ctr'])).astype(np.uint8).tolist())
    ...
    cv2.putText(show_img, 'pos', (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color['pos'])
    cv2.putText(show_img, 'neg', (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color['neg'])
    cv2.putText(show_img, 'ign', (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color['ign'])

    cv2.putText(show_img, dataset, (20, show_img_h-20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0))
    ...
    if len(distractor_boxes_recentered) > 0:
        for box in distractor_boxes_recentered:
            cv2.rectangle(show_img, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), color['neg'])
