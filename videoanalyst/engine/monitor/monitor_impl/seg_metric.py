...
from .utils import (AverageMeter, fast_hist, labelcolormap, mask_colorize, np_sigmoid)
...
class SegMetric(MonitorBase):
    ...
    def update(self, engine_data: Dict):
        ...
        if iter % self._hyper_params["interval"] != 0:
            ...
            # data
            seg_mask = engine_data["training_data"][self._hyper_params["gt_name"]]
            images = engine_data["training_data"][self._hyper_params["img_name"]]
            ...
            result_dict = {}
            for i, (image, ro, lt) in enumerate(zip(images, predict_data, seg_mask)):
                ...
                lp[ro > 0.5] = 1
                lp = lp.squeeze()
                hist += fast_hist(lp.flatten(), lt.flatten(), num_classes, ignore_label)
                ...
                if self._hyper_params["show_predict"]:
                    ...
                    show_image = self._draw_predict_mask(image_show, lp, num_classes)
                    cv2.imwrite(os.path.join(result_dir, "predict{}.png".format(self.show_id)),
                                show_image)
                if self._hyper_params["show_gt"]:
                    image_show = image.transpose(1, 2, 0).astype(np.uint8)
                    show_image = self._draw_predict_mask(image_show, lt, num_classes)
                    cv2.imwrite(os.path.join(result_dir, "gt{}.png".format(self.show_id)),
                                show_image)
                self.show_id += 1
            ...
            for show_key in self._hyper_params["show_items"]:
                if show_key not in self.outputs:
                    self.outputs[show_key] = AverageMeter(self._hyper_params["avg_range"])
