...
def calc_IoU(bbox1, bbox2):
    ...
    :return:
    """
    area1 = np.abs(bbox1[..., 2] - bbox1[..., 0] + 1) * np.abs(bbox1[..., 3] - bbox1[..., 1] + 1)
    area2 = np.abs(bbox2[..., 2] - bbox2[..., 0] + 1) * np.abs(bbox2[..., 3] - bbox2[..., 1] + 1)

    iw = np.minimum(bbox1[..., 2], bbox2[..., 2]) - np.maximum(bbox1[..., 0], bbox2[..., 0]) + 1
    ih = np.minimum(bbox1[..., 3], bbox2[..., 3]) - np.maximum(bbox1[..., 1], bbox2[..., 1]) + 1
...
def cxywh2xywh(box):
    ...
    return np.concatenate([box[..., [0]]-(box[..., [2]]-1)/2,
                           box[..., [1]]-(box[..., [3]]-1)/2,
                           box[..., [2]], box[..., [3]]], axis=-1)
...
def xywh2cxywh(rect):
    ...
    return np.concatenate([rect[..., [0]]+(rect[..., [2]]-1)/2,
                           rect[..., [1]]+(rect[..., [3]]-1)/2,
                           rect[..., [2]], rect[..., [3]]], axis=-1)
...
def xyxy2xywh(bbox):
    ...
    return np.concatenate([bbox[..., [0]], bbox[..., [1]],
                           bbox[..., [2]]-bbox[..., [0]]+1,
                           bbox[..., [3]]-bbox[..., [1]]+1], axis=-1)


def xywh2xyxy(rect):
    rect = np.array(rect, dtype=np.float32)
    return np.concatenate([rect[..., [0]], rect[..., [1]],
                           rect[..., [2]]+rect[..., [0]]-1,
                           rect[..., [3]]+rect[..., [1]]-1], axis=-1)
...
bbox_transform_test_cases = [
    dict(xyxy=(10., 20., 50., 40.),
         xywh=(10., 20., 41., 21.),
         cxywh=(30., 30., 41., 21.),
         ),
    dict(xyxy=(40., 40., 60., 60.),
         xywh=(40., 40., 21., 21.),
         cxywh=(50., 50., 21., 21.),
         ),
    dict(xyxy=(40., 60., 45., 75.),
         xywh=(40., 60., 6., 16.),
         cxywh=(42.5, 67.5, 6., 16.),
         ),
    dict(xyxy=(40., 60., 40., 60.),
         xywh=(40., 60., 1., 1.),
         cxywh=(40, 60., 1., 1.),
         ),
...
format_cvt_pairs = [(src, dst) for (src, dst) in itertools.product(formats, formats) if src != dst]
var_dict = locals()
...
class TestBboxTransform(unittest.TestCase):
    ...
    def test_bbox_transform(self):
        for src_fmt, dst_fmt in format_cvt_pairs:
            ...
            # for case in bbox_transform_test_cases:
            case_inputs = np.array([case[src_fmt] for case in bbox_transform_test_cases])
            case_answers = np.array([case[dst_fmt] for case in bbox_transform_test_cases])
            case_outputs = func_cvt(case_inputs)
            for out, ans in zip(case_outputs.reshape(-1), case_answers.reshape(-1)):
