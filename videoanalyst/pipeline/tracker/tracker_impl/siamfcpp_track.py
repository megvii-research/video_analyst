...
class SiamFCppTracker(PipelineBase):
    ...
    def update_params(self):
        ...
        hps['score_size'] = (hps['x_size'] - hps['z_size']) // hps['total_stride'] + 1 - hps['head_shrink']
        hps['score_offset'] = (hps['x_size'] - 1 - (hps['score_size'] - 1) * hps['total_stride']) // 2
    ...
    def feature(self, im, target_pos, target_sz, avg_chans=None):
        ...
        im_z_crop, _ = get_crop(im, target_pos, target_sz, z_size,
                                avg_chans=avg_chans, context_amount=context_amount,
                                func_get_subwindow=get_subwindow_tracking,)
        phase = self._hyper_params['phase_init']
        with torch.no_grad():
            features = self.model(imarray_to_tensor(im_z_crop).to(self.device), phase=phase)
    ...
    def track(self, im_x, target_pos, target_sz, features, update_state=False, **kwargs):
        ...
        im_x_crop, scale_x = get_crop(im_x, target_pos, target_sz, z_size,
                                      x_size=x_size, avg_chans=avg_chans, context_amount=context_amount,
                                      func_get_subwindow=get_subwindow_tracking,)
        with torch.no_grad():
            score, box, cls, ctr, *args = self.model(imarray_to_tensor(im_x_crop).to(self.device),
                                                     *features, phase=phase_track)
        ...
        best_pscore_id, pscore, penalty = self._postprocess_score(score, box_wh, target_sz, scale_x)
        # box post-processing
        new_target_pos, new_target_sz = self._postprocess_box(best_pscore_id, score, box_wh,
                                                              target_pos, target_sz, scale_x, x_size, penalty)

        if self.debug:
            box = self._cvt_box_crop2frame(box_wh, target_pos, x_size, scale_x)

        # restrict new_target_pos & new_target_sz
        new_target_pos, new_target_sz = self._restrict_box(new_target_pos, new_target_sz)
    ...
    def update(self, im):
        ...
        target_pos, target_sz = self.track(im, target_pos_prior, target_sz_prior, features,
                                           update_state=True)
    ...
    def _postprocess_score(self, score, box_wh, target_sz, scale_x):
        ...
        s_c = change(sz(box_wh[:, 2], box_wh[:, 3]) / (sz_wh(target_sz_in_crop)))  # scale penalty
        r_c = change((target_sz_in_crop[0] / target_sz_in_crop[1]) / (box_wh[:, 2] / box_wh[:, 3]))  # ratio penalty
    ...
    def _postprocess_box(self, best_pscore_id, score, box_wh, target_pos, target_sz, scale_x, x_size, penalty):
        """
    ...
    def _restrict_box(self, target_pos, target_sz):
        ...
        target_sz[0] = max(self._hyper_params['min_w'], min(self._state['im_w'], target_sz[0]))
        target_sz[1] = max(self._hyper_params['min_h'], min(self._state['im_h'], target_sz[1]))
    ...
    def _cvt_box_crop2frame(self, box_in_crop, target_pos, scale_x, x_size):
        ...
        """
        x = (box_in_crop[..., 0]) / scale_x + target_pos[0] - (x_size // 2) / scale_x
        y = (box_in_crop[..., 1]) / scale_x + target_pos[1] - (x_size // 2) / scale_x
