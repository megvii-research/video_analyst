...
def db_eval_iou(annotation, segmentation, void_pixels=None):
    ...
    assert annotation.shape == segmentation.shape, \
        'Annotation({}) and segmentation:{} dimensions do not match.'.format(annotation.shape,segmentation.shape)
    annotation = annotation.astype(np.bool)
    segmentation = segmentation.astype(np.bool)

    if void_pixels is not None:
        assert annotation.shape == void_pixels.shape, \
            'Annotation({}) and void pixels:{} dimensions do not match.'.format(annotation.shape,void_pixels.shape)
    ...
    if annotation.ndim == 3:
        ...
        for frame_id in range(n_frames):
            void_pixels_frame = None if void_pixels is None else void_pixels[
                frame_id, :, :, ]
            f_res[frame_id] = f_measure(segmentation[frame_id, :, :, ],
                                        annotation[frame_id, :, :],
                                        void_pixels_frame,
                                        bound_th=bound_th)
    elif annotation.ndim == 2:
        f_res = f_measure(segmentation,
                          annotation,
                          void_pixels,
                          bound_th=bound_th)
    else:
        raise ValueError(
            'db_eval_boundary does not support tensors with {} dimensions'.
            format(annotation.ndim))
    return f_res
...
def _seg2bmap(seg, width=None, height=None):
    ...
    assert not (width > w | height > h | abs(ar1 - ar2) > 0.01
                ), "Can" "t convert %dx%d seg to %dx%d bmap." % (w, h, width,
                                                                 height)
