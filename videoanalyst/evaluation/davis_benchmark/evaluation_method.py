...
def davis2017_eval(davis_path,results_path,csv_name_global_path,csv_name_per_sequence_path,hp_dict,version = '2017' ):
    time_start = time()
    ...
    dataset_eval = DAVISEvaluation(davis_root=davis_path, task=task, gt_set=dataset,version = version)
    metrics_res = dataset_eval.evaluate(results_path)
    J, F = metrics_res['J'], metrics_res['F']

    # Generate dataframe for the general results
    g_measures = ['J&F-Mean', 'J-Mean', 'J-Recall', 'J-Decay', 'F-Mean', 'F-Recall', 'F-Decay'] + hp_keys
    final_mean = (np.mean(J["M"]) + np.mean(F["M"])) / 2.
    g_res = np.array(
        [final_mean, np.mean(J["M"]), np.mean(J["R"]), np.mean(J["D"]), np.mean(F["M"]), np.mean(F["R"]),
         np.mean(F["D"])] + hp_values  )
    ...
    # Print the results
    sys.stdout.write("--------------------------- Global results for {} ---------------------------\n".format(dataset))
