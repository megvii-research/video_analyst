...
def calculate_failures(trajectory):
    ...
    """
    failures = [i for i, x in zip(range(len(trajectory)), trajectory) if len(x) == 1 and x[0] == 2]
...
def calculate_accuracy(pred_trajectory, gt_trajectory, burnin=0, ignore_unknown=True, bound=None):
    ...
    overlaps = region.vot_overlap_traj(pred_trajectory_[:min_len], gt_trajectory[:min_len], bound)
...
def success_error(gt_center, result_center, thresholds, n_frame):
    ...
    dist[mask] = np.sqrt(np.sum(np.power(gt_center[mask] - result_center[mask], 2), axis=1))
...
def determine_thresholds(scores, resolution=100):
    ...
    idxs = np.floor(np.linspace(delta - 1,
...
def calculate_expected_overlap(fragments, fweights):
    ...
    for i in range(1, max_len):
        ...
        if np.any(mask):
            ...
            expected_overlaps[i] = np.sum(seq_mean * fweights[mask]) / np.sum(fweights[mask])
