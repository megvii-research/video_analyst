...
class ExperimentGOT10k(object):
    ...
    def run(self, tracker, tracker_name="Untitled", visualize=False, save_video=False):
        if self.subset == 'test':
        ...
        print('Running tracker %s on GOT-10k...' % tracker_name)
        ...
        for s, (img_files, anno) in enumerate(self.dataset):
            ...
            for r in range(self.repetitions):
                ...
                if r > 0 and tracker.is_deterministic:
                    break
                elif r == 3 and self._check_deterministic(tracker_name, seq_name):
                    print('  Detected a deterministic tracker, ' + 'skipping remaining trials.')
                    break
                print(' Repetition: %d' % (r + 1))

                # skip if results exist
                record_file = os.path.join(self.result_dir, tracker_name, seq_name,
