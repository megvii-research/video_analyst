...
class GOT10k(object):
    ...
    def __init__(self, root_dir, subset='test', return_meta=False,
        ...
        if os.path.isfile(cache_path) and not ignore_cache:
            ...
            for seq_name in tqdm(self.seq_names):
                ...
                GOT10k.data_dict[seq_name] = dict(img_files = img_files, anno=anno, meta=meta)
            with open(cache_path, "wb") as f:
                pickle.dump(GOT10k.data_dict, f)
    ...
    def load_single_sequence(self, seq_dir):
        r"""        
        Args:
            seq_dir (string): path of  a sequence.
    ...
    def __getitem__(self, index):
        ...
        if self.return_meta:
            meta = GOT10k.data_dict[seq_name]["meta"]
            return img_files, anno, meta
        else:
